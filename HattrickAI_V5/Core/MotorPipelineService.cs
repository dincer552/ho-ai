...
public sealed class MotorPipelineService
{
    ...
    private static M6FormationSearchBudget BudgetFromRank(int rank, int formationCount, int baseBeamWidth, int baseIterations)
    {
        ...
        return tier switch { 0 => new M6FormationSearchBudget(Math.Max(baseBeamWidth, 4), Math.Max(baseIterations, 2)), 1 => new M6FormationSearchBudget(Math.Max(baseBeamWidth, 3), Math.Max(baseIterations, 2)), _ => new M6FormationSearchBudget(Math.Max(2, baseBeamWidth - 1), Math.Max(1, baseIterations - 1)) };
    }
