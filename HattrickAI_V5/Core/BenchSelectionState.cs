...
public sealed record BenchPlayerOption(
    int Id,
    string Name,
    string[] EligibleSlots);

public sealed record BenchSelectionPayload(
    IReadOnlyList<BenchRecommendation> Recommendations,
    IReadOnlyList<BenchPlayerOption> Candidates);
...
public static class BenchSelectionState
{
    ...
    public static void Save(string ownerKey, IReadOnlyList<Player> players, IReadOnlyList<Slot> finalXi)
    {
        ...
        var candidates = players
            .Where(p => !xiIds.Contains(p.Id))
            .Select(p => new BenchPlayerOption(p.Id, p.Name, EligibleSlots(p).ToArray()))
            .OrderBy(p => p.Name, StringComparer.OrdinalIgnoreCase)
            .ThenBy(p => p.Id)
            .ToList();
        State[ownerKey] = new BenchSelectionPayload(recommendations, candidates);
    }
