...
[JsonConverter(typeof(LineupJsonConverter))]
...
public sealed class LineupJsonConverter : JsonConverter<Lineup>
{
    public override Lineup Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var doc = JsonDocument.ParseValue(ref reader);
        var root = doc.RootElement;
        var teamName = root.TryGetProperty("teamName", out var team) ? team.GetString() ?? string.Empty
            : root.TryGetProperty("TeamName", out team) ? team.GetString() ?? string.Empty : string.Empty;
        var formation = root.TryGetProperty("formation", out var form) ? form.GetString() ?? string.Empty
            : root.TryGetProperty("Formation", out form) ? form.GetString() ?? string.Empty : string.Empty;
        if (!root.TryGetProperty("slots", out var slotsElement) && !root.TryGetProperty("Slots", out slotsElement))
            throw new JsonException("Lineup slots bulunamadı.");
        var slots = JsonSerializer.Deserialize<IReadOnlyList<Slot>>(slotsElement.GetRawText(), options)
            ?? throw new JsonException("Lineup slots çözümlenemedi.");
        return new Lineup(teamName, formation, slots);
    }

    public override void Write(Utf8JsonWriter writer, Lineup value, JsonSerializerOptions options)
    {
        writer.WriteStartObject();
        writer.WriteString("teamName", value.TeamName);
        writer.WriteString("formation", value.Formation);
        writer.WritePropertyName("slots");
        JsonSerializer.Serialize(writer, value.DisplaySlots, options);
...
{
    public RatingCalculationTraceResult? OwnRatingCalculation { get; init; }
