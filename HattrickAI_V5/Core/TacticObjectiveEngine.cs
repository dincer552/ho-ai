...
public static class TacticObjectiveEngine
{
    ...
    {
        var passing = Average(i.TotalPassing, count);
        var defending = Average(i.TotalDefending, count);
        var scoring = Average(i.TotalScoring, count);
        var stamina = Average(i.TotalStamina, count);
        var experience = Average(i.TotalExperience, count);
        return tactic switch
        {
    ...
    {
        var ownLoss = RelativeLoss(baseline.OwnRegularChanceExpected, c.OwnRegularChanceExpected);
        var opponentGain = RelativeLoss(baseline.OpponentRegularChanceExpected, c.OpponentRegularChanceExpected);
        return tactic switch
        {
    ...
    private static double Suppression(M8ChanceResult c, M8ChanceResult baseline)
        => baseline.OpponentRegularChanceExpected <= 1e-9 ? 0 : Clamp01(1.0 - c.OpponentRegularChanceExpected / baseline.OpponentRegularChanceExpected);
...
public sealed record TacticFitResult(
    TeamTactic Tactic,
    double FitScore,
...
public sealed record ComparisonEvaluationView(
    M8ChanceResult Chance,
    AdvancedTacticalScenarioResult Advanced,
    M9PredictionResult Prediction);
