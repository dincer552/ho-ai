...
public static class FoxtrickEngineStage4Regression
{
    ...
    public static int Run()
    {
        ...
        // Pure Foxtrick formula regression. All seven sectors are fixed at 10.
        var synthetic = new RegionalRatingSnapshot(
        ...
        // Engine contract regression on a deterministic eleven-player lineup.
        ...
        var slots = new[] { "GK", "DEF-L", "DEF-CL", "DEF-C", "DEF-CR", "W-L", "IM-L", "IM-C", "IM-R", "W-R", "FW-C" }
            .Select((code, i) => new Slot(code, code, "stage4", players[i].Name, players[i].Id, 0, 0, 0))
            .ToList();
        ...
        // Real CHPP fixture lock: Foxtrick sectors use the canonical regional source;
        // therefore their sector values must remain identical to the locked V5 raw snapshot.
        ...
        var lineup = new Lineup(
            ...
            fixtureSlots);
        var context = RatingContext.Default;
        var request = new RatingEngineRequest(lineup, fixturePlayers, context);
        ...
        for (var i = 0; i < expectedHo.Length; i++)
            CheckNear(hoValues[i], expectedHo[i], 1e-9, $"HO sector {i}", failures);
