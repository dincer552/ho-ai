...
if (From(1)) { var r = WebInputIntegrityRegression.Run(); if (r != 0) return r; r = CoreWebParityRegression.Run(); if (r != 0) return r; r = FoxtrickPositionRegression.Run(); if (r != 0) return r; r = await M3M11EndToEndRegression.RunAsync(path); if (r != 0) return r; }
